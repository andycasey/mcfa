...
data_kwds = dict(n_samples=N, n_features=D, n_latent_factors=J,
                 n_components=K, lkj_eta=1, noise_scale=1,
...
def generate_data(n_samples, n_features, n_latent_factors, n_components,
                  lkj_eta=2, noise_scale=1, latent_scale=1, random_seed=0):
...
# Try init from true position?
init_from_true = {
    "lambda": np.sort(truth["pi"]),
    "xi": truth["xi"].T,
    "beta_lower_triangular": truth["beta_lower_triangular"],
    "beta_diag": truth["beta_diag"],
    "psi": truth["psi"],
    "sigma_L": truth["sigma_L"],
    "Omega_corr": np.eye(J).reshape((1, J, J)),
    "Omega_diag": np.diag(truth["omega"][:, :, 0]).reshape((1, J))
}


model = stan.load_model("mcfa.stan")

...
\newcommand{\ExperimentHash}{8761b}
...
%[0000-0003-0174-0564]
\author{Some combination (TBD) of: Andrew R. Casey}
...
We find that five latent factors are preferred to explain 18 chemical elements
...
$\alpha$-element production, and a fifth latent factor where only K and Zn
dominate. We discuss examples where the inferred latent factors deviate from 
expected yields, which includes an  indication that Sc is associated with the r-process.
We find three clusters are preferred given 1,072 stars in \Galah,
...
of latent factors, components, and $\sim10^6$ abundance measurements.
\end{abstract}
...
\end{equation}
\noindent{}are drawn from\footnote{For clarifying nomenclature across disciplines, the terminology e$z \sim \mathcal{N}(0, 1)$ indicates that the variable $z$ is drawn from a standard normal distribution.} the $\numcomponents$th multivariate normal distribution.
...
\subsection{Toy model with generated data} \label{sec:exp-1}
...
\begin{figure*}
	...
	\caption{Latent factors inferred from the \Galah\
			 data \citep[][; thick lines]{Buder:2018}. The panels on the left show the absolute entries for each
			 ...
			 to each element, ordered by the loads that contribute most to each
			 element.}

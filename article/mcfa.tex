...
\begin{equation}
	{I}(\vec\Psi,\vecdata) = \frac{Q}{2}\log\kappa\left(Q\right) - \log\left(\frac{p(\vec\Psi)}{\sqrt{|\mathcal{F}(\vec\Psi)|}}\right) - \log\mathcal{L}\left(\vecdata|\vec\Psi\right)% + \frac{Q}{2}
\end{equation}
...
Finally, the $\log\kappa(Q)$ term is given by
\begin{equation}
	\todo{\log\kappa} \quad .
\end{equation}
...
parameters. \todo{Dowe said something about MML reducing to BIC under some conditions, but I have not yet found the right paper to cite.}
...
such that the optimal lossless message to encode them is \citep[p279, sec. 6.8.2][]{someone},
\begin{eqnarray}
	I(\NumLatentFactors) &= -\log{p(\NumLatentFactors)} &= \NumLatentFactors\log{2} + \textrm{constant} \\
	I(\NumComponents) &= -\log{p(\NumComponents)} &= \NumComponents\log{2} + \textrm{constant} \quad .
\end{eqnarray}
...
$\scorecovs$ \citep[Section 5.2.3][]{someone},
\begin{equation}
...
as $|\mathcal{F}(\scoremeans)||\mathcal{F}(\scorecovs)|$ \citep{25,27} where
\begin{equation}
...
&& \cdots \,\, -\frac{1}{2}(2\NumLatentFactors+3)\sum_{k=1}^{K}\log{|\scorecovs_k|}  \quad .
\end{eqnarray}
...
ensuring that the factor loads themselves remain mutually orthogonal \citep{Wallace}.
...
\begin{equation}
	p(A) = \frac{|\factorloads\transpose\factorloads|^{\frac{1}{2}(\NumDimensions - \NumLatentFactors - 1)}}{2^{\frac{\NumDimensions\NumLatentFactors}{2}}|\vec{S}|^{\frac{\NumDimensions}{2}}\Gamma(\frac{\NumDimensions}{2})}\exp\left[-\frac{1}{2}\textrm{Tr}(\vec{S}^{-1}\factorloads\transpose\factorloads)\right] \quad .
\end{equation}
...
\begin{eqnarray}
	...
	&=& -\log{p(\factorloads)} + \frac{1}{2}\log{|\mathcal{F}(\factorloads)|} \nonumber \\
...
Combining equations \ref{eq:all} leads to the full message length
\begin{equation}
\todo{here}
\end{equation}
...
\begin{figure*}
	\includegraphics[width=\textwidth]{experiments/exp1-compare-all-mml.pdf}
...
to a small degree for many elements \citep[e.g.,][]{Dotter_2008},

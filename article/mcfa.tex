...
data $\vecdataunscaled - \vec\mu$ (which we hereafter will refer to simply as $\vecdata$) can be described as \citep[Sec. 5.2.9, p236;][]{Wallace:2005}
\begin{equation}
...
orthogonal vectors.
%$\mathbf{H}$, and by taking $\factorloads_\ast$ to be the Cholesky decomposition of $\mathbf{H}\transpose \mathbf{H}$, we initialise the factor loads as
%\begin{eqnarray}
...
\noindent{}where $\odot$ denotes is the entry-wise product. 
%We repeat
...
% weights: K - 1 model parameters.
% psi: D model parameters
% A: J * D - J^2 model parameters (Baek et al. 2010)
% xi: J * K model parameters
% omega: K * J*(J+1)/2
...
term can be reasonably approximately by
\begin{equation}
...
%Still, the specific model scatter is far less than the observed scatter,

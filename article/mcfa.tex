...
in lower-dimensional latent space.
...
\consider{We find that five latent factors are preferred to explain 18 chemical elements
in the second \Galah\ data release.}
From these latent factors we identify the rapid- and slow-neutron capture 
processes, as well as latent factors consistent with Fe-peak and 
$\alpha$-element production, and a \consider{fifth latent factor where only K and Zn
dominate}. We discuss examples where the inferred latent factors deviate from 
expected yields, which includes an \consider{indication that Sc is associated with the r-process}.
\consider{We find three clusters are preferred given 1,072 stars in \Galah,
...
that adding a dimension of say [Ni/H] provides entirely new, independent information
...
missing data entries in order to update our estimates of the model parameters.
...
contamination by a cosmic ray), or various other reasons.
...
we impute zeros for missing data entries, and at each iteration we update
this imputed value with our estimate of what the missing data values
are, given the current model parameters. Similarly, with each update we inflate
our estimates of the specific variances $\vec\psi$ based on the fraction of missing
data points in each dimension
\begin{equation}
\specificvariance\nextstep = \specificvariance\nextstep\left(\frac{N}{N-\vec{M}}\right)
\end{equation}
\noindent{}where $\vec{M}$ is a vector that contains the number of missing data entries in
all $D$ dimensions.
...
of missing data, with varying fractions of missing data in each dimension.
...
section we describe a toy model using generated and real data, where we will assume
that the true number of latent factors and components are not known. 
Hence we require some heuristic to decide how many latent factors and components are 
...
For these reasons we also apply the Minimum Message Length \citep[MML;][]{Wallace:2005}
...
&& \cdots +\frac{Q}{2}\log\kappa_q +\frac{1}{2}\left[\NumLatentFactors(\NumDimensions + 2) + \NumComponents(2-\NumData)\right]\log{2}  \quad . \nonumber \label{eq:message-length}
 \end{eqnarray}
\end{widetext}
...
we recover the true data-generating parameters given some data, and to
ensure that the method is yielding consistent results.
...
We generated the factor loads in the same way that we initialise the optimisation (Section~\ref{sec:initialisation}). The relative weights $\vec\weight$
...
We recorded the \emph{negative} log likelihood, the BIC, and the message length (omitting constant terms) for each permutation of $\NumLatentFactors$ and $\NumComponents$.
...
and clustering in the latent space provides a better description of the (generated) data.
...
Here we repeat the toy model used in the previous Section, but we discard an increasing fraction
of the data and evaluate the model's performance in the presence
...
\begin{figure*}[t!]
    ...
    \caption{The estimated factor loads $\factorloads$ (left), factor scores $\factorscores$ (middle),
    		 and specific variances $\specificvariance$ (right) compared to the 
		 	 true data generating values
		 	 for Experiment~2. Here 40\% of the data are missing at random.
...
despite 40\% of the data entries to be missing.
...
\begin{figure*}
	...
	\caption{Detailed chemical abundances from \Galah\ \citep{Buder:2018} used in this work. Each star is coloured by its inferred component in latent space.}
...
closely to the target load (e.g., $\mathbf{L}_1$ which we identify as the s-process), and some barely
match at all (e.g., $\mathbf{L}_4$). Here we show the absolute entry of the factor loads because even
if an entry is negative, the corresponding factor scores could also be negative, and their product will contribute to the observed abundances. For this reason the factor load sign does not matter here.
...
\end{equation}
The fractional contributions to each element are shown in the right hand side of
Figure~\ref{fig:exp3-factor-loads}. As targeted, we identify the first factor $\mathbf{L}_0$ 
as being most similar to the r-process, and here it is the dominant contributor to Eu and La, both heavy elements that are primarily r-process tracers. Surprisingly we also find that this factor load is the dominant contributor to the Fe-peak element Sc. The specific scatter in Sc is 0.03~dex (Figure~\ref{fig:exp3-specific-scatter}), suggesting that the Sc abundances are well-described by this latent factor model.
 

The second latent factor $\mathbf{L}_1$ here is most representative of the slow neutron capture process, with dominant contributions to Ba and Y. This factor has some support at other elements, including Cu and Sc, but with very little effective contributions. Most Cu is contributed by $\mathbf{L}_2$, our Fe-peak latent factor. In fact $\mathbf{L}_2$ is the primary contributor to nearly all Fe-peak elements, with close to negligible contributions from other factors. The exception here is Zn, where slightly more contribution comes from $\mathbf{L}_4$ than it does from $\mathbf{L}_2$. Recall that while $\mathbf{L}_4$ was described as our `light odd-Z element' factor load, it is the dominant producer of both K and Zn, and secondary contributor to Ti, V, and to a lesser extent, La. The fourth latent factor $\mathbf{L}_3$ is better behaved. It is the dominant contributor to the $\alpha$-element tracers Si, Ca, and Ti, and nothing else. It contributes a reasonable fraction to Al and Zn, but with negligible contributions elsewhere.
The specific scatter after accounting for these latent factors is smallest
for Fe and largest for K (Figure~\ref{fig:exp3-specific-scatter}). The typical scatter is about 0.05~dex.

In Figure~\ref{fig:exp3-latent-space} we show the inferred clustering in latent space,
where the separation between components is arguably best seen in the splitting between $\factorscores_4$ with respect to $\factorscores_1$ or $\factorscores_2$. When projected to data space (Figure~\ref{fig:exp3-data-space}) the third component (purple) is seen to have relatively higher abundance ratios of [K,Ba,Zn/Fe] at a given [Fe/H],
and lower abundance ratios of [V/Fe]. This is consistent with the clustering in latent space, where $\factorloads_4$ is the dominant contributor to K and Zn, and $\factorloads_{1,2}$ are the dominant contributor to Ba and V, respectively.
...
missing. We assume those measurements are missing at random, and discuss the
...
procedure using 1,000 stars ($N =$ 2,072), then again with 10,000 ($N =$ 11,072),
...
\consider{For sample sizes up to $N = 11,072$ we find that the number of preferred latent
factors remains at $J = 5$. The factor loads inferred from each subset of stars
is shown in Figure~\ref{fig:exp10-comparison-loads}, illustrating that the
...
the loads to be as close to those inferred in Section~\ref{sec:exp4}.} inferred factor
loads remain largely unchanged despite the increasing sample size, with an increasing
fraction of missing data entries. It is not until the data set reaches $N = 11,072$ 
stars where we find that $J = 8$ factor loads are preferred by the data. Again, the
first five latent factors are extremely similar to those inferred from just 1,072
stars with complete abundances, except for the notable difference in Zn loads in
factor load $|\mathbf{L}_4|$, which disappears for $N = 11,072$ and re-appears
in the sixth factor load $|\mathbf{L}_5|$ -- indicative that the additional data
prefers the fourth latent factor to split into an additional latent factor where
Zn contributions dominate.}
...
\begin{figure*}[t!]
	...
	\caption{Latent factors found in Section~\ref{sec:exp10} using different subsets
	         of \Galah\ data \citep{Buder:2018}. The thin line shows the result from
	         Section~\ref{sec:exp4} with $N=1,072$ stars with 17 abundances and no
	         missing data. Dotted line shows that data set complemented with 100 random
	         stars that have some abundances missing. Dot-and-dashed line shows the
	         original 1,072 stars with 1,000 random stars with some missing abundances,
	         and the thick line shows the original 1,072 with 10,000 stars with
	         missing abundances, where eight latent factors were preferred over five.}
    \label{fig:exp10-comparison}
\end{figure*}


\todo{If we use ALL data, how many? Then just comment on clustering in the N = XXX,XXX data}
...
the mean of these yields would still require mutual orthogonality in this model. Introducing a constraint on the factor scores that resolves this indeterminacy and allows for more flexible latent factors would be a worthy extension to this work.
...
using \Galah\ data, we find that $\factorloads_4$ is the dominant contributor to
potassium, but this factor load is also the dominant contributor to Zn, 
an element that is typically associated with the iron-peak group. That factor load
is also a reasonable contributor to Ti, V, and to a lesser extent, La. Does this
...
to be high in [Zn/Fe] (for which $\factorloads_4$ also dominates), but there
...
of K are not as well described by the latent factor model as other
elements.


A similar argument could be made for Sc, where $\mathbf{L}_0$ -- a factor load we
identify as the r-process -- is the dominant contributor. Indeed, the second largest
contributor to Sc is the factor load we identify as the s-process. Sc is under-produced in 
...
We find five latent factors from \Galah\ data, and
...
is encouraging that there is some interpretability in the latent factors. Indeed, in the elements where we find surprising associations (e.g., Sc with what we identify as the r-process, K and Zn linked by a common process), these are elements where galactic chemical evolution models are most discrepant from observations, even after accounting for systematic errors in abundance measurements.
...
our model does provide the first data-driven approach to nucleosynthesis and chemical tagging.

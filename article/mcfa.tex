...
of latent factors, components, and  $\sim3 \times 10^7$ abundance measurements.
\end{abstract}
...
The Fisher information for $\factorloads$ \todo{needs to be checked, but the effect is small.}
...
\begin{figure*}
	...
	\caption{Latent factors inferred from the \Galah\
			 ...
			 factor load, where the thin lines indicate the target latent factors. On the right we show the absolute fractional contributions
...
The model with $J = 5$ and $K = 3$ is found to have the shortest message length, which we take as our preferred model for these data.
...
\begin{itemize}
	...
	\item The third factor load should have non-zero entries in Fe-peak elements.
...
We find that five latent factors from \Galah\ data, and

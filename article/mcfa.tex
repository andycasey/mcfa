...
\end{equation}
%\todo{\noindent{}and the Message Length.}
...
Given our estimate of the number of latent factors and components ($\NumComponents = 21$, $\NumLatentFactors = 5$), in Figure~\ref{fig:exp1-latent} we show the estimated latent
space (factor scores $\factorscores$) where each point is coloured by it's inferred
associated component. After applying an orthogonal rotation (see following section)
to the estimated latent space, we show the estimated factor scores (Figure~\ref{fig:exp1-factor-scores-comparison}) and estimated factor loads (Figure~\ref{fig:exp1-factor-loads-comparison})
compared to the true values. 
Similarly, in Figure~\ref{fig:exp1-specific-variance} we show the estimated specific scatter
in each data dimension compared to the true value. 
\todo{The agreement is excellent in all model parameters.}
...
\begin{figure*}
    ...
    \caption{The inferred latent scores $\factorscores$ for Experiment~1. 
             Points are coloured by the inferred associated component.}
...
%\footnote{Clifford
%algebra offers a more efficient algorithm that is more suitable for high dimensions, 
%but we found Givens rotation matrices to be suitable for our purposes here.}
In Figure
...
% TODO: matches Barklem variances?

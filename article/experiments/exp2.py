...
# Sort by periodici table.
ei = np.argsort(np.array([periodic_table.index(el) for el in use_elements]))
use_elements = [use_elements[_] for _ in ei]
_ = [ea.strip() for ea in list(elements)]
use_element_indices = np.array([_.index(el) for el in use_elements])


use_star_indices = np.all(np.isfinite(X[:, use_element_indices]), axis=1)

X = X[use_star_indices, :][:, use_element_indices]

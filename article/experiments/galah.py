...
data = Table.read(os.path.join(here, "GALAH_iDR3_OpenClusters.fits"))
...
available_elements = list(set([ea.split("_")[1][:2].rstrip("123456789") \
                        for ea in data.dtype.names if ea.startswith("A_")]))
...
def _abundance_flag_label(element):
    return f"flag_A_{element}"




def get_abundance_mask(elements, cluster_names=None, use_galah_flags=False,
                       full_output=False):
    ...
    if cluster_names is None or len(cluster_names) < 1:
        ...
        use = np.zeros(len(data))

    for cluster_name in cluster_names:
        use += (data["wg4_field"] == cluster_name)
...
def get_abundances_breakdown(elements, cluster_names, use_galah_flags=False):

    use, counts = get_abundance_mask(elements, cluster_names=cluster_names, 
...
def get_abundances_wrt_h(elements, cluster_names=None, use_galah_flags=False):
    ...
    use = get_abundance_mask(sorted_elements, cluster_names, 
                             use_galah_flags=use_galah_flags)

    X_H = np.array([data[_abundance_label(el)][use] - asplund_2009[el] \

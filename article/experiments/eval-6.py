...
comparison_model_paths = [
    "eval-figs/eval-5-models-size-1000.pkl",
    "eval-figs/eval-5-models-size-10000.pkl",
    "eval-figs/eval-5-models-size-157242.pkl"
]

label_names = ['na_h', 'mg_h', 'al_h', 'si_h', 'k_h', 'ca_h', 'sc_h', 'ti_h', 'mn_h', 'fe_h', 'ni_h', 'cu_h', 'zn_h', 'y_h', 'ba_h', 'la_h', 'eu_h']
...
plot_factor_loads_kwds = dict(colors=colors, separate_axes=True, lw=1, xlabel=r"$\textrm{element}$", alpha=0,
                              xticklabels=[r"$\textrm{{{0}}}$".format(ea.split("_")[0].title()) for ea in label_names])

fig = mpl_utils.plot_factor_loads(viz_load(_fill_factors(A_astrophysical, J_max)), 
                                  figsize=(6.75, 9.50),
...
plot_kwds = {
    comparison_model_paths[0]: dict(linestyle=":", lw=1, alpha=1, zorder=10),
    comparison_model_paths[1]: dict(linestyle="-.", lw=2, alpha=0.75, zorder=1),
    comparison_model_paths[2]: dict(linestyle="-", lw=5, alpha=0.5, zorder=-10),

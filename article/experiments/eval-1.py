...
label_names = [f"$\\mathbf{{S}}_{{{i}}}$" for i in range(data_kwds["n_latent_factors"])]
...
gridsearch_max_latent_factors = int(2 * J)
gridsearch_max_components = int(2 * K)
...
# Save grid search output in case we need it in future.
suffix = "gridsearch.pkl"
prefix = os.path.basename(__file__)[:-3]
here = os.path.dirname(os.path.realpath(__file__))
filename = os.path.join(here, "eval-figs",  f"{prefix}-{suffix}")

contents = dict(Jm=Jm, Km=Km, Y=Y, mcfa_kwds=mcfa_kwds,
                J_grid=J_grid, K_grid=K_grid, converged=converged, metrics=metrics)

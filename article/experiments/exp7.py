...
X_all = np.array([data[f"{el}_H"] for el in elements]).T
...
Ks = np.array([64, 128, 256, 512])

path_gridsearch = f"{prefix}-gridsearch-results.pkl"

if not os.path.exists(path_gridsearch):
    Jg, Kg, converged, metrics  = grid_search.grid_search(Js, Ks, X, N_inits=5,
                                                                     mcfa_kwds=mcfa_kwds,
                                                                     suppress_exceptions=True)

    with open(path_gridsearch, "wb") as fp:
        pickle.dump((Jg, Kg, converged, metrics, X, mcfa_kwds), fp)

else:
    with open(path_gridsearch, "rb") as fp:
...
t = data
_, factor_scores, __, tau = model.factor_scores(X_all)
for i in range(model.n_latent_factors):
    t[f"S_{i}"] = factor_scores[:, i]


t["component"] = np.argmax(tau, axis=1)
t["used_in_model"] = np.zeros(len(t), dtype=bool)
t["used_in_model"][mask] = True


for j in range(model.n_components):
    t[f"C_{j}"] = -1
    t[f"C_{j}"][mask] = (t["component"][mask] == j)


stripped_fields = np.array([field.strip() for field in t["FIELD"]])
t["M15_member"] = (stripped_fields == "M15") \
                * (t["FE_H"] < -1.6) \
                * (t["VHELIO_AVG"] > -130) \
                * (t["VHELIO_AVG"] < -90)

t["M53_member"] = (stripped_fields == "M53") \
                * (t["FE_H"] < -1.5) \
                * (t["VHELIO_AVG"] > -90) \
                * (t["VHELIO_AVG"] < -50)

t["M92_member"] = (stripped_fields == "M92") \
                * (t["FE_H"] < -1.7) \
                * (t["VHELIO_AVG"] > -200) \
                * (t["VHELIO_AVG"] < -50)
